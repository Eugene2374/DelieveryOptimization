...
def createList(csv,createdClass):
    ...
    return pd.Series([createdClass(item) for item in itemCSV])

def centre(List):
    return len(List)>0 and listFDiv(len(List),listSum(*[items.pos for items in List])) or [0,0]
...
def listSum(*args):
    ...
    for items in args:
        for index,values in enumerate(items):
            if len(total)==index:
                total.append(values)
            elif len(total)<index:
                raise ValueError('List Not Continuous')
            else:
                total[index]+=values

    return total

def listMult(value,list1):
    return [value*i for i in list1] ## Can put value=-1 for negative values

def listFDiv(value,list1):
    return [i//value for i in list1] 
