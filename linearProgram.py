...
class Simulator:
    def __init__(self):
        ### Initiating
        global logRiders,plot, MerchantList,RiderList,CustomerList,orderTimes
        from main import logRiders, plot
        ...
        while not all([customer.done for customer in CustomerList]):
            ...
            if time in orderTimes:
                for rider,order in choice[time]:
...
def fCostMatrix(state:dict,currTime,depth):
    orderList=[order for order in CustomerList if currTime==order.appearTime]
    if depth==0:
        CostMatrix= pd.DataFrame({order:[state[rider][0].dist(order.merchant)+order.plannedTime+state[rider][1] for rider in RiderList] for order in orderList})
        choice={}
    else:
        nextTime=orderTimes[orderTimes.index(currTime)+1] if currTime in orderTimes and currTime<max(orderTimes) else currTime
        CostMatrixDict={}
        for order in orderList:
            CostList=[]
            for rider in RiderList:
                dummy=state.copy()
                dummy.update({rider:[order,state[rider][0].dist(order.merchant)+order.plannedTime+state[rider][1]]})
                choice,delay=fCostMatrix(dummy,nextTime,depth-1)
                TotalTime=delay+state[rider][0].dist(order.merchant)+order.plannedTime+state[rider][1]
                CostList.append(TotalTime)
            CostMatrixDict.update({order:CostList})
        CostMatrix=pd.DataFrame(CostMatrixDict)
    
    minCostR,minCostO= linear_sum_assignment(CostMatrix)
    choice.update({currTime:zip([RiderList[rider] for rider in minCostR],[orderList[order] for order in minCostO])})
    return (choice,sum(CostMatrix.loc[costR,orderList[costO]] for costR,costO in zip(minCostR,minCostO)))
